...
export class EntryController {
  static async sentiment(req: Request, res: Response<IResponse>) {
    ...
    const { userId, entryId, sentimentScore } = req.body;
    ...
    const doc = await updateOne(
      ...
      {
        $set: {
          "journals.$.sentimentScore": sentimentScore,
        },
      }
    );
    ...
    if (doc.modifiedCount === 0) {
      return res
        ...
        .json(userResponses.could_not_update("Journal entry sentiment score could not be updated."));
    }
